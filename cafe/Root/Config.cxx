...
namespace cafe {
    ...
    TEnv& Config::getEnv(bool force_rereading)
    {
        static TEnv *_env;

        if(_env == 0 || force_rereading) {
            // Search order:
    ...
    void Config::resolve(bool force_rereading)
    {
        TEnv& env(getEnv(force_rereading));
